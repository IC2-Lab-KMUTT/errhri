"""T1 Direction-A (v2) — fuse BOTH temporal sets into the calibrated stack:
  - traj temporal OOF  (temporal_oof_t1.csv,        T_  prefix) — 32-frame blendshapes
  - dense temporal OOF (temporal_dense_oof_t1.csv,  TD_ prefix) — 48-frame au_seq+gaze_seq
over the 6-stream tabular zoo. Isotonic-calibrated greedy forward selection; official eval.
Reference: 6-stream stack 0.685 | traj-fused best 0.696.
"""
...
for fn, pref in [(f"temporal_oof_t{track}.csv", "T_"), (f"temporal_dense_oof_t{track}.csv", "TD_")]:
    fp = CACHE_DIR / fn
    if not fp.exists():
...
def official(pf, tag):
    ...
    d = tempfile.mkdtemp(prefix="t1fd_")
...
official(fuse({k: P[k] for k in chosen}), f"A2_greedy {chosen}")
...
print("\nReference: stack6 0.685 | traj-fused 0.696", flush=True)
